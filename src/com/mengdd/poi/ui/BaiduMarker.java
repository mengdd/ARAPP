...
public class BaiduMarker extends BasicMarker
{
			...
			BDLocation origBDLocation)
	{
		...
		GeoPoint mOrigGeoPoint = BaiduLocationHelper.getGeoPointFromBDLocation(origBDLocation);
		// Compute the relative position vector from user position to POI
		// location
		mLocationVector = MathUtils
				.convGeoPointToVector(mOrigGeoPoint, mGeoPoint);
	...
	@Override
	protected void updateDistance(Location origLocation, BDLocation origBDLocation)
	{
		if (null == origBDLocation)
		{
			throw new IllegalArgumentException("origBDLocation is null!");
