...
public class CompassViewModel extends ViewModel implements LocationListener {
    ...
    private void updateOrientation(float[] values) {
        if (compassView != null) {
            ...
            mCurrentSecond = c.get(Calendar.SECOND);

            int milliseconds = c.get(Calendar.MILLISECOND);
            // 按照时间输出，查看滤波前后效果，绘制曲线
            LogUtils.i(TAG, "isAntiAlias: " + isAntiAlias + ", time: "
                    + mCurrentSecond + " s, " + milliseconds + " ms"
                    + ", bearing: " + values[0]);
            LogUtils.logToFile(TAG, "isAntiAlias: " + isAntiAlias + ", time: "
                    + mCurrentSecond + " s, " + milliseconds + " ms"
                    + ", bearing: " + values[0]);

            if (mCurrentSecond != mLastSecond) {
                mLastSecond = mCurrentSecond;
                ++mSecondCount;
                LogUtils.logToFileOne(mSecondCount + "." + milliseconds);
                LogUtils.logToFileTwo(Float.toString(values[0]));
            }
